...
def q():
    ...
    avg_gen_before = request.form.get("avg_before")
    avg_gen_after = request.form.get("avg_after")
    if (avg_gen_before != None or avg_gen_after != None):
        if (avg_gen_before != None):
            safe_request.add_avg_request(request_builder.fields["gen_before"])
        if (avg_gen_after != None):
            safe_request.add_avg_request(request_builder.fields["gen_after"])
    
    request_str = request_builder.build_select_request(safe_request)
    lib_rows = postgre_repository.select(request_str)
    decrypted = request_builder.decrypt_rows(lib_rows, safe_request)
    if (avg_gen_before != None or avg_gen_after != None):
        sum_before = None
        sum_after = None
        count = None
        ...
        avg_after = None
        if (avg_gen_before != None):
            avg_before = decrypted[0][0]
        if (avg_gen_after != None):
            after_index = 0
            if (avg_gen_before != None):
                after_index = 1
                count_index = 2
            avg_after = rows[0][after_index]
        item = Item()
        item.count = count
    ...
    for row in decrypted:
        ...
        item.name = row[1]
        item.age = row[2]
        item.therapy_duration = row[3]
        item.gen_before = row[4]
        item.gen_after = row[5]
        item.is_effective = row[6]
        items.append(item)
...
def select_all_test(size):
    ...
    decrypt_time = 0
    cypher_time = 0
    db_request.he_pub = he_cipher.pub.n * he_cipher.pub.n
    for i in range(size):
        age = 1
        #name = 'khvduqqeot'
        db_request.avg_gen_before = 1
        ...
        #encrypted_name = str_to_number(det_encrypt_string(name))
        cypher_time += (time.time() - ts)
        db_request.max_age = encrypted
        #db_request.name = encrypted_name
        ts = time.time()
        rows_2 = select(db_request, "test_2")
        enc_time += (time.time() - ts)
        if len(rows_1) != len(rows_2):
